...
module.exports = class WorkScheduleItemTest extends BaseSeleniumTest {
    constructor(driver) {
        super(driver);
        this.exampleItem = {
            "name": "sdfdsvrfd",
    ...
    async navigateToList() {
        ...
        await this.clickElement("a[href=\"/WorkScheduleItem\"]")
        await this.asleep(1000);
        await this.takeScreenshot("WorkScheduleItem-list-before");
        for (const name in this.exampleItem) {
            const value = this.exampleItem[name];
            if (typeof value === "string" && value.length > 5) {
                expect(await this.driver.findElement(By.css(".page-WorkScheduleItem-list")).getText()).to.not.contain(value);
            }
        }
    }
    ...
    async addNew(item, assert) {
        ...
        await this.clickElement("a[href=\"/WorkScheduleItem/add\"]")
        await this.asleep(1000);
        await this.takeScreenshot("WorkScheduleItem-add-before", assert);
        for (const name in item) {
            const value = item[name];
            await this.sendKeysToElement("form [name=\"" + name + "\"]", value);
        }
        await this.asleep(200);
        await this.takeScreenshot("WorkScheduleItem-add-filled", assert);
        await this.clickElement("form button");
        await this.asleep(2000);
        await this.takeScreenshot("WorkScheduleItem-afterAdd", assert);
    }
};