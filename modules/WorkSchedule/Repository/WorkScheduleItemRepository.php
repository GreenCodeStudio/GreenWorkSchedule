...
class WorkScheduleItemRepository extends \Core\Repository
{
    ...
    public function getDataTable($options)
    {
        ...
        $rows = DB::funquery("
SELECT wsi.id, wsi.start, wsi.end, json_object('id',wsi.user_id, 'name',u.name, 'surname',u.surname, 'color', up.value) as user 
...
JOIN user u ON u.id = wsi.user_id 
    ...
    {
        return DB::get("SELECT * FROM work_schedule_item WHERE user_id = ? AND date(start) BETWEEN ? AND ? ORDER BY start", [$workerId, $startRange, $endRange]);
    }

    public function getById(int $id)
    {
        $item = DB::get("SELECT wsi.*, json_object('id',u.id, 'name',u.name, 'surname',u.surname) as user
FROM work_schedule_item wsi
JOIN user u ON u.id = wsi.user_id
         WHERE wsi.id = ?", [$id])[0] ?? null;
        if($item) {
            $item->user = json_decode($item->user);
        }
        return $item;
    }
}
